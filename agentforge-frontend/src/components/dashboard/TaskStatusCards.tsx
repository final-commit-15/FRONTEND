...
const statusConfig = [
  { key: 'pending', label: 'Pending', icon: Clock, color: 'text-info-500', bg: 'bg-info-100', bar: 'bg-info-500' },
  { key: 'in_progress', label: 'In Progress', icon: AlertCircle, color: 'text-brand-primary', bg: 'bg-brand-primary/10', bar: 'bg-brand-primary' },
  { key: 'in_review', label: 'In Review', icon: PauseCircle, color: 'text-violet-500', bg: 'bg-violet-100', bar: 'bg-violet-500' },
  { key: 'completed', label: 'Completed', icon: CheckCircle2, color: 'text-success-500', bg: 'bg-success-100', bar: 'bg-success-500' },
  { key: 'blocked', label: 'Blocked', icon: AlertTriangle, color: 'text-error-500', bg: 'bg-error-100', bar: 'bg-error-500' },
] as const;
  ...
  return (
    <section className="card p-6">
      <div className="flex items-center justify-between mb-5">
        <h3 className="font-semibold text-text-heading">Tasks by Status</h3>
      ...
      <div className="mt-5 pt-4 border-t border-canvas-border flex items-center justify-between">
        <div>
          <p className="text-xs text-text-muted mb-0.5">Total Tasks</p>
