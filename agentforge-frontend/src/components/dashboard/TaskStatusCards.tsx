// src/components/dashboard/TaskStatusCards.tsx

import { cn } from '../../lib/utils';
import { AlertCircle, Clock, CheckCircle2, PauseCircle, AlertTriangle } from 'lucide-react';
...
const statusConfig = [
  { key: 'pending', label: 'Pending', icon: Clock, color: 'text-info-500', bg: 'bg-info-100' },
  { key: 'in_progress', label: 'In Progress', icon: AlertCircle, color: 'text-brand-primary', bg: 'bg-brand-primary/10' },
  { key: 'in_review', label: 'In Review', icon: PauseCircle, color: 'text-violet-500', bg: 'bg-violet-100' },
  { key: 'completed', label: 'Completed', icon: CheckCircle2, color: 'text-success-500', bg: 'bg-success-100' },
  { key: 'blocked', label: 'Blocked', icon: AlertTriangle, color: 'text-error-500', bg: 'bg-error-100' },
] as const;

export function TaskStatusCards({ summary }: TaskStatusCardsProps) {
  return (
    <div className="card p-6">
      <h3 className="font-semibold text-text-heading mb-4">Tasks by Status</h3>
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        {statusConfig.map(({ key, label, icon: Icon, color, bg }) => {
          const count = summary[key as keyof TaskStatusSummary] as number;
          ...
          return (
            <div key={key} className="card p-4 hover:shadow-md transition-shadow">
              ...
              <div className="text-2xl font-bold text-text-heading">{summary[key as keyof TaskStatusSummary]}</div>
              <div className="text-xs text-text-muted">{label}</div>
              <div className="mt-2 h-1.5 bg-canvas-surface rounded-full overflow-hidden">
                <div
                  className="h-full bg-brand-primary rounded-full transition-all duration-300"
                  style={{ width: `${summary.total > 0 ? (summary[key as keyof TaskStatusSummary] as number) / summary.total * 100 : 0}%` }}
                />
              </div>
            </div>
          );
